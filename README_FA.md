<div dir="rtl" align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/version-0.2.1-22C8E6?style=for-the-badge" alt="نسخه">
...
<div align="center">
  <img src="assets/demo.gif" alt="دموی دوسو" width="800">
</div>
...
دوسو هر ۷ روز یک‌بار (به‌صورت پیش‌فرض) نسخه‌های جدید را از GitHub Releases بررسی می‌کند، شبیه oh-my-zsh. این بررسی هرگز شروع برنامه را کند نمی‌کند: یک فایل کش کوچک را می‌خواند، در صورت قدیمی بودن نسخه یک پیام کوتاه نشان می‌دهد، و آن کش را در پس‌زمینه برای دفعهٔ بعد به‌روز می‌کند. اگر نسخهٔ جدیدتری پیدا شود:

```
⚠ A new version of dosu is available: 0.2.0 (you have 0.1.0). Run `dosu update` or see https://github.com/RustNegar/dosu/releases
```

برای دیدن دستور مناسب ارتقا بر اساس روش نصب‌تان:

```bash
dosu update
```

برای غیرفعال‌کردن کامل این بررسی (مفید در محیط‌های محدود/آفلاین/CI):

```bash
export DOSU_DISABLE_UPDATE_CHECK=1
```

می‌توان این را از طریق `config.toml` هم غیرفعال کرد یا بازهٔ زمانی آن را تغییر داد (به [پیکربندی](#پیکربندی) نگاه کنید).
...
تنظیمات در `~/.config/dosu/config.toml` قرار دارند (فقط اگر خودتان آن را بسازید). موارد مربوط به بررسی به‌روزرسانی:

```toml
update_check_enabled = true       # برای غیرفعال‌کردن false کنید (یا از DOSU_DISABLE_UPDATE_CHECK=1 استفاده کنید)
update_check_interval_days = 7    # فاصلهٔ بررسی GitHub Releases
```
