<div dir="rtl" align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/version-0.2.1-22C8E6?style=for-the-badge" alt="גרסה">
...
<div align="center">
  <img src="assets/demo.gif" alt="הדגמת Dosu" width="800">
</div>
...
Dosu בודק מעת לעת (כל 7 ימים כברירת מחדל) אם קיימת גרסה חדשה יותר ב-GitHub Releases, בדומה ל-oh-my-zsh. הבדיקה לעולם לא מעכבת את ההפעלה: היא קוראת קובץ מטמון (cache) קטן, מציגה הודעה קצרה אם הגרסה שלכם ישנה, ומרעננת את המטמון ברקע לפעם הבאה. אם נמצאה גרסה חדשה יותר:

```
⚠ A new version of dosu is available: 0.2.0 (you have 0.1.0). Run `dosu update` or see https://github.com/RustNegar/dosu/releases
```

הריצו `dosu update` כדי לראות את פקודת השדרוג המתאימה לאופן שבו התקנתם:

```bash
dosu update
```

כדי לבטל את הבדיקה לחלוטין (שימושי בסביבות מוגבלות/לא מקוונות/CI):

```bash
export DOSU_DISABLE_UPDATE_CHECK=1
```

ניתן גם לבטל אותה, או לשנות את התדירות, דרך `config.toml` (ראו [הגדרות](#הגדרות)).
...
ההגדרות נמצאות ב-`~/.config/dosu/config.toml` (נוצר רק אם תיצרו אותו בעצמכם). הרלוונטיות לבדיקת העדכונים:

```toml
update_check_enabled = true       # קבעו false כדי לבטל (או השתמשו ב-DOSU_DISABLE_UPDATE_CHECK=1)
update_check_interval_days = 7    # כל כמה ימים לבדוק מול GitHub Releases
```
